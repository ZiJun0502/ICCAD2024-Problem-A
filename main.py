...
def main():
    ...
    while session_end - session_start <= 600:
        ...
        Phase 5: Gate Sizing
        """
        gate_sizing_ga = GateSizingGA(
            n=params['gs_ga_n'],
            n_init=params['gs_ga_n_init'],
            n_iter=params['gs_ga_n_iter'],
            # design_path=epoch_best_netlist,
            design_path="playground/design6-cost_estimator_1/a.v",
            # design_path=best_buffer_netlist,
            output_path=params['output_path'],
            # init_population=[ga_min_cell_map],
            session_min_cost=session_min_cost,
            id_dim_limit=[len(library.cell_map[gate_type]) for gate_type in library.gate_types],
        )
        best_gate_sizing_netlist, gs_min_cost = gate_sizing_ga.run()
