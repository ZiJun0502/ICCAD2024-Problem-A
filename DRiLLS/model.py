...
from DRiLLS.mapper import Mapper
def log(message):
...
class A2C:
    def __init__(self, n_episodes, n_iter):
        self.n_episodes = n_episodes
        self.n_iter = n_iter
        # init session
        self.game = abcSession()
        ...
        self.state_input = tf.compat.v1.placeholder(tf.float32, [None, self.state_size])

        # Define any additional placeholders needed for training your agent here:
        self.actions = tf.compat.v1.placeholder(tf.float32, [None, self.num_actions])
    ...
    def optimizer(self):
        ...
        """
        return tf.compat.v1.train.AdamOptimizer(0.01).minimize(self.loss_val)        
    ...
    def loss(self):
        ...
        # actor loss        
        neg_log_prob = tf.nn.softmax_cross_entropy_with_logits_v2(logits=tf.math.log(self.actor_probs), 
                                                                  labels=self.actions)
    ...
    def save_best_netlist(self):
        best_cost = self.game.best_known_cost[0]
        best_episode = self.game.best_known_cost[1]
        best_iteration = self.game.best_known_cost[2]

        best_netlist_path = os.path.join(self.game.params['playground_dir'], str(best_episode), \
            str(best_iteration) + '_abc_mapped.v')
        best_netlist_unmapped_path = os.path.join(self.game.params['playground_dir'], str(best_episode), \
            str(best_iteration) + '_abc.v')
        unmapped_output_path = self.game.params['output_file'].replace('.v', '_abc.v')
        with open(best_netlist_path, 'r') as src:
            with open(self.game.params['output_file'], 'w') as dest:
                dest.write(src.read())
        with open(best_netlist_path, 'r') as src:
            with open(self.game.params['output_file'].replace('.v', '_copy.v'), 'w') as dest:
                dest.write(src.read())
        with open(best_netlist_unmapped_path, 'r') as src:
            with open(unmapped_output_path, 'w') as dest:
                dest.write(src.read())
        return best_cost
    def train(self):
        all_rewards = []
        for i in range(self.n_episodes):
            ...
            log('done with total reward = ' + str(total_reward))
            print('')
    def train_episode(self):
        ...
        for i in range(self.n_iter):
            ...
            # end = time.time()
            # print(f"action: {end - start}")
            # start = time.time()
            new_state, reward = self.game.step(action)
            # end = time.time()
            # print(f"step: {end - start}")
            log('  iteration: ' + str(self.game.iteration) + '\n'\
                '  cost: ' + str(self.game.cost) + \
                '  action: ' + self.game.params['actions'][action] + '\n')
            ...
            state = self.normalizer.normalize(state).eval(session=self.session)
            # end = time.time()
            # print(f"observe, eval: {end - start}")
        ...
        log(f'Episode Agent Training Time: {end - start:.5f}) second(s).')
        
        best_cost = self.game.best_known_cost[0]
        best_episode = self.game.best_known_cost[1]
        best_iteration = self.game.best_known_cost[2]
        log("Best cost: " + str(best_cost) +\
        " at episode: " + str(best_episode) +\
        " at iteration: " + str(best_iteration))
